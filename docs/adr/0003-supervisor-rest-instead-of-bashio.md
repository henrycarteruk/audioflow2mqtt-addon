...
This keeps the entire add-on in one language on a slim Python base with no
bashio/shell indirection (see ADR 0004 for the base image and tooling choice).
Recorded because bashio is the idiomatic default; without this note someone would
likely "fix" the absence of bashio by reintroducing it.
